...
$productArr = $item->gettallyData($start_date, $end_date);

$totalQuantity = 0;
$totalAmount = 0;

foreach ($productArr as &$row) {
  $row["price"] = is_numeric($row["price"]) ? (float)$row["price"] : 0.0;
  $row["quantity"] = is_numeric($row["quantity"]) ? (int)$row["quantity"] : 0;
...
$template = $twig->loadTemplate("administer/item/salestally.twig");
